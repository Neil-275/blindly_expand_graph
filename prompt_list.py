...
extract_relation_prompt = """Please retrieve %s relations (separated by semicolon) that contribute to the question and rate their contribution on a scale from 0.0 to 1.0 (the sum of the scores of %s relations is 1). Give your answer follow exactly to the format in the examples. Never truncate or shorten relations. Every relation must be written in its full canonical form as provided in the relations list, including all slashes and trailing segments.
