...
def calculate_query_coverage(query, sampling_method, method_name):
    ...
    try:
        ...
        if len(answers) == 0:
            ...
            topk_node_set = set(topk_nodes) if topk_nodes is not None else set()
            intersection = answers & topk_node_set
            coverage = len(intersection) / len(answers)
            hit = 1 if len(intersection) > 0 else 0
        
        # Handle subgraph size
        if subgraph is not None and len(subgraph) > 0:
            # Ensure subgraph has the right shape
            if hasattr(subgraph, 'shape') and len(subgraph.shape) >= 2:
                unique_nodes = np.unique(subgraph[:, [0, 2]].flatten())
                subgraph_size = len(unique_nodes)
            else:
                subgraph_size = len(subgraph) if hasattr(subgraph, '__len__') else 0
        else:
            subgraph_size = 0
...
def test_method_on_queries(queries, sampling_method, method_name, max_queries=50):
    ...
    for i, query in enumerate(queries):
        try:
            ...
            subgraph_sizes.append(metrics['subgraph_size'])
            successful_queries += 1
        except Exception as e:
            print(f"\n     ⚠️ Error processing query {i+1}: {str(e)}")
            continue
