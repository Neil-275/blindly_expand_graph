    ...
    for epoch in range(epochs):
        ...
        for anchors, pos_stack, neg_stacks in train_loader:
            ...
            for i_layer in range(num_layers):
                proj_a = projector(i_layer, anchors) # [B, GNN_Dim]
                pos = pos_stack[:, i_layer, :]       # [B, GNN_Dim]
                ...
                for k in range(num_negatives):
                    neg = neg_stacks[:, k, i_layer, :] # [B, GNN_Dim]
        ...
        with torch.no_grad():
            for anchors, pos_stack, neg_stacks in val_loader:
                ...
                batch_l = 0
                for i_layer in range(num_layers):
                    proj_a = projector(i_layer, anchors)
                    pos = pos_stack[:, i_layer, :]
                    lnl = sum([criterion(proj_a, pos, neg_stacks[:, k, i_layer, :]) for k in range(num_negatives)])
