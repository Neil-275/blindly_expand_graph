...
class ExpandSubgraphFreebase:
    ...
    def __init__(
        ...
        # Initialize other utitlity variables here if needed
    ...
    def assign_query(
        self,
        query: dict,
        question_key: str = 'natural_query',
        start_entities_key: str = 'topic_entity',
        crucial_triples_key: str = 'mid_crucial_triples'
    ):
        self.query = query
        ...
        self.start_entities: list[str]  # List of entity IDs (MIDs) that are the starting points for subgraph expansion

        topic_entities_dict: dict[str, str] = query.get(start_entities_key, {})
        for mid, entity_name in topic_entities_dict.items():
            self.mid2name[mid] = entity_name
            self.start_entities.append(mid)

        if self.GoG_simulation:
            # self.edge_index = self.orignal_edge_index.copy()
            print("Simulating GoG by removing edges...")
            # if query['gold_path'] is None:
            #     self.remove1hopEdges()
            #     self.removeGoldRelationPath()
            # else:
            #     self.removeEdges()
            # self.updateEdges(self.edge_index)
            self.drop_edges = set[tuple[str, str, str]]
            mid_crucial_triples_list = query.get(crucial_triples_key, [])
            for mid_head, relation, mid_tail in mid_crucial_triples_list:
                self.drop_edges.add((mid_head, relation, mid_tail))
    ...
    def compare_rel_query_and_return_topk(self, triplets: list,  fuse_func=fuse_mean, ):
        ...
        unique_rel_desc = np.unique(triplets[:, 1])
        # rels_emb = ExpandSubgraph.rel_embs[unique_rel_ids]

        # rels_emb = np.apply_along_axis(
        #     lambda rel_desc: self.model.encode(rel_desc, convert_to_tensor=True).to(self.args.device),
        #     1,
        #     unique_rel_desc.reshape(-1, 1)
        # )
        rels_emb = self.model.encode(unique_rel_desc, convert_to_tensor=True).to(self.args.device)

        scores = self.util.dot_score(self.query_emb, rels_emb).to('cpu')
        top_k_indices = self.return_top_k(scores, fuse_func=fuse_func)
        # DONE HERE
        selected_rels = unique_rel_desc[top_k_indices]

        rel_order = {unique_rel_desc[idx]: rank for rank, idx in enumerate(top_k_indices)}
    ...
    def sampleSubgraph(self, mode="train"):
        assert (self.query), "Please assign a query first using assign_query()"
        ...
        while len(start_entities) > 0:
            ...
            for head_id in start_entities_copy:
                ...
                filterd_edges = [
                    edge
                    for edge in edges
                    if (edge[0], edge[1], edge[2]) not in self.visited
                ]
                triplets.extend(filterd_edges)
            ...
            for triplet in np_triplets:
                ...
                # self.visited_relations.add(triplet[1])
                # self.visited_entities.add(triplet[2])

                # Update dict here
                relation_name = triplet[1]
                if relation_name not in self.id2rel.values():
                    self.id2rel[len(self.id2rel)] = relation_name

                if triplet[0] not in self.mid2name:
                    _entity_name = freebase_interface.convert_id_to_name(triplet[0])
                    self.mid2name[triplet[0]] = _entity_name

                    if _entity_name not in self.id2name.values():
                        self.id2name[len(self.id2name)] = _entity_name

                if triplet[2] not in self.mid2name:
                    _entity_name = freebase_interface.convert_id_to_name(triplet[2])
                    self.mid2name[triplet[2]] = _entity_name
            ...
            start_entities = set()
            for triplet in np_triplets:
                if triplet[2] != head_id:  # Avoid adding the same head as a new start entity
                    start_entities.add(triplet[2])
                elif triplet[0] != head_id:  # If the tail is the same as head, consider adding the head as a new start entity
                    start_entities.add(triplet[0])
            start_entities = list(set(start_entities))
        ...
        # TODO ????
        node_index = torch.zeros(
            # len(ExpandSubgraph.id2ent),
            5,  # Placeholder: Adjust this to the actual number of entities in your graph
            dtype=torch.long
        )
