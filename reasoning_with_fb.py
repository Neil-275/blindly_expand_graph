...
from model_2 import Projector, GNN_auto

load_dotenv()


class ReasoningModuleFreebase:
    def __init__(
        self,
        ...
        # Attributtes for reasoning a query
        self.result: list[str] = None  # Final answer(s) after reasoning
        self.query: dict = None

        self.entIndex2name: dict[int, str]  # Mapping from entity local ID its name
        self.id2rel: dict[int, str]  # Mapping from relation local ID to its description

        self.question_key: str = question_key
    ...
    def assign_query(self, query: dict):
        ...
        self.sampler.assign_query(
            ...
            start_entities_key=self.start_entities_key,
            crucial_triples_key=self.crucial_triples_key,
        )
        # global_subraph_data = self.sampler.sampleSubgraph()
        ...
        # self.gnn_model.assign_rel_emb(self.rel_emb)
        # self.adj_list = self.build_adjacency_list()
        self.start_entities = self.sampler.start_entities

        self.entIndex2name, self.id2rel = self.sampler.id2name, self.sampler.id2rel

    # def assign_dict(
    #     self,
    #     id2ent: dict[int, str],
    #     id2rel: dict[int, str],
    #     rel_desc_dict: dict[int, str]  # This is the natural language description dict for relations
    # ):
    #     self.entIndex2name, self.id2rel = id2ent, id2rel
    #     self.ent2id = {v: k for k, v in id2ent.items()}
    #     self.rel2id = {v: k for k, v in id2rel.items()}
    #     self.rel_desc_dict = rel_desc_dict
    #     # Dummy: Delete later
    #     # TODO: Write the interface to map all global id to local indexing
    ...
    #     # self.rel_emb = self.construct_rel_emb(self.subgraph_edges)
    #     self.rel_emb = self.create_relation_embeddings()
    #     self.gnn_model.assign_rel_emb(self.rel_emb)
    #     self.adj_list = self.build_adjacency_list()

    def _get__start_entities_from_query(self, query: dict):
        # DONE
        ...
        labels: list[str] = flatten_nested_tuple(query.get("query_type", []))
        values: list[str] = flatten_nested_tuple(query.get("raw_query", []))
        return [
    ...
    def construct_rel_emb(self, subgraph_edges):
        rel_emb = get_subgraph_relation_embeddings(subgraph_edges, self.rel_desc_dict)
    ...
    def create_relation_embeddings(self):
        self.id2rel = sorted(self.id2rel.items(), key=lambda x: x[0])  # Sort by relation ID
        relation_embeddings = torch.stack(
            [
                get_last_token_embedding(self.rel_desc_dict[r_id])
                for r_id, _ in self.id2rel
            ]
        )
    ...
    def _entity_id_to_name(self, entity_id: str):
        # NEW
        if entity_id in self.entIndex2name:
            return entity_id
        entity_name: str = freebase_interface.convert_id_to_name(entity_id)
        self.entIndex2name[entity_id] = entity_name
    ...
    def _trace_to_triplet_strings(self, trace):
        ...
        """
        return [
            f"{self.entIndex2name[h]}, {r}, {self.entIndex2name[t]}"
            for h, r, t in trace
        ]
    ...
    def _trace_to_history_string(self, trace):
        ...
        return f"{self.entIndex2name[h]} --> {r} --> {self.entIndex2name[t]}"

    # ------------------------------------------------------------------
    # Step 1 – JUDGE
    # ------------------------------------------------------------------

    def call_llm_judge(self, question: list[str], knowledge_triplets):
    ...
    def reasoning(self):
        # question: list[str] = self.query.get('natural_query', None)
        raw_question = self.query.get(self.question_key, None)
        if isinstance(raw_question, str):
            ...
            raise ValueError(
                f"Question format not recognized. Expected str or list[str], got {type(raw_question)}"
            )
        if len(question) == 0:
            raise ValueError(
                "Query does not contain the "
            )
